...
func (rf *Raft) InstallSnapshot(args *InstallSnapshotArgs, reply *InstallSnapshotReply) {
	...
	if rf.IsLogExist(args.StartLogIndex, args.StartLogTerm) {
		...
		Debug(dSnap, "[Snapshot][S%v]{Follower} Discard Entire log", rf.me)
	}

	// Debug(dSnap, "[Snapshot][S%v]{Follower}, old start is %v, length of log is %v, new start is %v", rf.me, rf.logStart, len(rf.logs), args.StartLogIndex)
	// Debug(dLog, "[S%v]{Follower} index is %v", rf.me, args.StartLogIndex)
	Debug(dLog, "[Snapshot][S%d]{Follower} log(Term) becomes: %q", rf.me, rf.GetTermArray())
